...
class PredictCompute():
    ...
    def path_predict(self, path, v, N):
        ...
        trend_theta=0.0
        delta2_theta=0.0
        pred_range=min(len(path),N)
        for i in range(pred_range):
            future_theta_err=path[i][2]-path[i][2]

        # 曲率確認
        for i in range(len(path)-1):
            delta_theta = self.compute_theta_error(path[i][2],path[i+1][2])
            delta_theta_list.append(delta_theta)
        if len(delta_theta_list)>1:
            delta2_theta=self.compute_theta_error(delta_theta_list[0],delta_theta_list[1])
    ...
    def compute_omega_error(self, state, path):
        x, y, theta, delta, v, accel = state
        # 曲率確認
        if len(path)>1:
            delta_theta = self.compute_theta_error(path[0][2],path[1][2])
            omega_error=self.compute_theta_error(delta_theta, delta)
        else:
            omega_error=0.0
        return omega_error

    def get_path_data(self, x_ref, y_ref, reference_trajectory, N):
        # 現在の速度を元に取得する経路情報を計算
        section=N*2
        # 現在位置から最も近いポイントを探す
        x_ref, y_ref, ref_index = self.physics.find_closest_point(x_ref, y_ref, reference_trajectory)
    ...
    def adjust_target_angle(self, theta_ref, theta_target):
        """
        theta_ref の方向に最も近い theta_target を求める
        """
        theta_target_norm = self.normalize_angle(theta_target)

        # ±180° をまたぐ場合、基準に近い方向に寄せる
        if abs(theta_target_norm - theta_ref) > np.pi:
            if theta_target_norm > theta_ref:
                theta_target_norm -= 2 * np.pi
            else:
                theta_target_norm += 2 * np.pi

        return theta_target_norm

    def compute_theta_error(self, theta_ref, theta_target):
        """
        角度誤差を計算（正しく -π ~ π の範囲に補正）
        """
        # 目標角度を現在の向きに最も近い形に補正
        theta_target_adjusted = self.adjust_target_angle(theta_ref, theta_target)

        # 角度誤差を計算
        theta_error = self.normalize_angle(theta_target_adjusted - theta_ref)
